...
export async function sesBounceWorker(jobs: Job<Payload>[]): Promise<void> {
  for (const job of jobs) {
    try {
      ...
      // Mark the event as processed unconditionally — fires even when recipients
      // is empty so zero-recipient envelopes are correctly marked idempotent.
      await db
      ...
      logger.error(
        { event: 'worker_job_failed', job_id: job.id, worker: 'sesBounce', error: String(err) },
