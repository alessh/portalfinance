...
result: pending
note: "Previously failed (testcontainers cascade + env-assert good-path). Plan 02-09 pinned the integration project to singleFork + isolate:false, refactored startTestDb() to a globalThis singleton, and registered a globalSetup that boots one shared container. Wall time dropped from 232s to 19s, zero cascade markers. Plan 02-08 fixed the env-assert good-path fixture. Re-run."
...
result: pending
note: "Was blocked on Test 1 env-loading blocker (now closed by 02-07). Re-run."
...
result: pending
note: "Was blocked on the testcontainers cascade (now closed by 02-09). Re-run."
...
result: pending
note: "Was blocked on the testcontainers cascade and Test 1 env-loading (both now closed by 02-09 + 02-07). Re-run."
...
result: pending
note: "Was blocked on testcontainers cascade and Test 1 env-loading (both now closed by 02-09 + 02-07). Re-run."
...
- truth: "Opening /connect renders the Pluggy Connect entry point without runtime errors; environment variables required by the web runtime (NODE_ENV, DATABASE_URL, NEXTAUTH_SECRET, ENCRYPTION_KEY, CPF_HASH_PEPPER) are loaded before module evaluation; server-only env schema is not evaluated in client bundles."
  ...
  resolved_note: "02-07 added `import 'server-only'` to env.ts and crypto.ts and split cpf.ts into isomorphic + server-only modules so the client bundle no longer reaches the env loader. 02-08 documented the .env.local bootstrap and added docs/ops/local-dev-setup.md. Manual smoke (UAT Test 1) still required."
...
- truth: "Webhook idempotency: posting the same Pluggy event 3× yields a single webhook_events row, no double sync; invalid X-Pluggy-Signature returns 401. Verified by tests/integration/pluggy/webhook.test.ts under live Postgres."
  status: resolved
  resolved_by: [02-09-PLAN.md, 02-08-PLAN.md]
  resolved_note: "02-09 pinned the integration vitest project to singleFork + isolate:false, made startTestDb() a globalThis singleton, and added a globalSetup to boot one shared Postgres testcontainer. Cascade markers eliminated; wall time 19s vs 232s. 02-08 fixed the env-assert good-path fixture. Plus a follow-up commit (`8c9793f`) added `vi.mock('server-only', () => ({}))` to tests/setup.ts so vitest can resolve modules guarded by 02-07's server-only import. Manual re-run (UAT Test 2) still required to confirm webhook.test.ts is green end-to-end."
