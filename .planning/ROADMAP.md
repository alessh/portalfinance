...
- [ ] 02-11-PLAN.md — Wave 1 (reviews gap-closure / closes #1+#11): redact pluggy_item_id from webhook_events.payload AND pg-boss job payloads (new redactPluggyPayload helper); workers consume item_id_hash_hex; retention + non-exposure + log-prohibition policy doc (criterion #6 verifiable)
- [ ] 02-12-PLAN.md — Wave 2 (reviews gap-closure / closes #3): move item/login_succeeded inline DB-lookup + recordAudit out of webhook hot path into new PLUGGY_REAUTH_AUDIT worker (idempotent on webhook_event_id); webhook-latency regression test (<100ms median)
- [ ] 02-13-PLAN.md — Wave 1 (reviews gap-closure / closes #4): TransferDetector deterministic mutual best match (ROW_NUMBER tie-break) — 1-to-1 invariant under one-debit-many-credits ambiguity; 3 new determinism tests
- [ ] 02-14-PLAN.md — Wave 1 (reviews gap-closure / closes #5): FaturaDetector best-effort — Pluggy creditData.balanceDueDate preferred anchor + accounts.updated_at fallback; multi-card-ambiguity guard (no flag if 2+ cards match); 5 false-positive tests + best-effort spec doc; [BLOCKING] schema push for accounts.bill_due_date
- [ ] 02-15-PLAN.md — Wave 2 (reviews gap-closure / closes #6+#7): centralized isSyncableItemStatus + needsReauth helpers (single source of truth for item-state policy); add DISCONNECTED enum value; disconnect route atomically transitions status='DISCONNECTED'; sync/reconcile/UI consume helpers (no more direct string comparisons); [BLOCKING] schema push for item_status enum
- [ ] 02-16-PLAN.md — Wave 1 (reviews gap-closure / closes #8+#10): empirical pg-boss singleton-semantics test (5 jobs same key + singletonHours=0 → ≤2 completions); vi.doMock route-stability test under singleton testcontainer; canonical test-conventions doc
- [ ] 02-17-PLAN.md — Wave 3 (reviews gap-closure / closes #2+#13): nightly + on-demand GitHub Actions workflow runs real-Pluggy-sandbox e2e (60s assertion for criterion #1); Playwright screenshot smoke for /transactions (empty/loaded/paywall) + /settings/connections (healthy/broken/cooldown); audit-trail file auto-committed; ops runbook (autonomous: false — human checkpoint to populate GitHub Secrets)
- [ ] 02-18-PLAN.md — Wave 1 (reviews gap-closure / closes #12): add pluggy_items.last_manual_sync_at column; cooldown anchor switches to manual-only — failed manual syncs DO NOT cool down, recent webhook syncs DO NOT block manual sync; semantic deliberate per Concern #12; [BLOCKING] schema push
